...
public class TraitRegistry {
    ...
    public static void activateHellmare(Player player, Horse horse) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                Location loc = horse.getLocation().clone();
                Block below = loc.subtract(0, 1, 0).getBlock();

                if (below.getType().isSolid()) {
                    loc.getWorld().spawnParticle(Particle.FLAME, horse.getLocation(), 5, 0.3, 0.3, 0.3, 0.01);
                    below.setType(Material.FIRE);
                }

                ticks++;
                if (ticks >= duration * 20) cancel();
            }
    ...
    public static void activateFireheart(Player player, Horse horse) {
        ...
        player.sendMessage(ChatColor.RED + "Fireheart activated! 🔥");

        horse.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, duration * 20, 0));
    ...
    public static void activateFeatherHooves(Player player, Horse horse) {
        ...
        player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, duration * 20, 0));
        player.sendMessage(ChatColor.GREEN + "Feather Hooves activated!");
    }
    ...
    public static void activateGhostHorse(Player player, Horse horse) {
        ...
        horse.addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, duration * 20, 0));
    ...
    public static void activateFrostHooves(Player player, Horse horse) {
        Block under = horse.getLocation().subtract(0, 1, 0).getBlock();
        if (under.getType() == Material.WATER) {
            under.setType(Material.FROSTED_ICE);
            player.sendMessage(ChatColor.BLUE + "Frost Hooves froze the water!");
        }
    }

    public static void activateWindStep(Player player, Horse horse) {
        Block under = horse.getLocation().subtract(0, 1, 0).getBlock();
        if (under.getType().toString().contains("GRASS")) {
            double speed = horse.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).getBaseValue();
            double multiplier = getConfig().getDouble("traits.windstep.speed-multiplier", 1.2);
            horse.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).setBaseValue(speed * multiplier);
            ...
            }.runTaskLater(BetterHorses.getInstance(), 5 * 20L);
        }
    }
